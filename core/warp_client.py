...
from warp2protobuf.config.settings import (
    ...
    OS_VERSION,
    REFRESH_TOKEN_B64,
    REFRESH_URL as CONFIG_REFRESH_URL,
    has_http2_support,
...
from core.tool_trace import (
    ...
    summarize_warp_event,
    truncate_text,
)
from core.tool_compat import normalize_tool_calls
...
class WarpClient:
    ...
    REFRESH_URL = CONFIG_REFRESH_URL
    ...
    @staticmethod
    def _format_httpx_exception(exc: Exception) -> str:
    ...
    def _log_trace(self, trace_id: Optional[str], event_name: str, **payload: Any) -> None:
        if not trace_id:
            return
        log_tool_trace(event_name, trace_id, account=self.account.name, **payload)
    ...
    async def _post_with_transport_retry(
        ...
        *,
        ...
        attempt_http2_modes = [True, False] if has_http2_support() else [False]
        last_error: Exception | None = None

        for attempt_idx, http2_enabled in enumerate(attempt_http2_modes, start=1):
            try:
                async with httpx.AsyncClient(
                    **get_httpx_client_kwargs(
                    ...
                    response = await client.post(url, headers=headers, content=content, json=json_body)
                    if attempt_idx > 1:
                        logger.info(
                            "%s recovered for '%s' on retry (attempt %s, http2=%s)",
                            purpose,
                            self.account.name,
                            attempt_idx,
                            http2_enabled,
                ...
                last_error = exc
                logger.warning(
                    "%s transport error for '%s' (attempt %s/%s, http2=%s, url=%s): %s",
                    purpose,
                    self.account.name,
                    attempt_idx,
                    len(attempt_http2_modes),
                    http2_enabled,
                    url,
                    self._format_httpx_exception(exc),
                )
                if attempt_idx >= len(attempt_http2_modes):
                    raise
                await asyncio.sleep(0.4 * attempt_idx)
        ...
        try:
            ...
            if response is not None:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        "Token refresh failed for '%s': HTTP %s, body=%s",
                        self.account.name,
            ...
            logger.error(
                "Token refresh transport error for '%s': %s",
                self.account.name,
                self._format_httpx_exception(e),
            )
            return False
            # 超时不标记账户状态，可能是网络问题
            return False
            ...
            logger.error(
                "Error refreshing token for '%s': %s",
                self.account.name,
                self._format_httpx_exception(e),
            )
            return False
    ...
    async def _acquire_anonymous_token(self) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(**get_httpx_client_kwargs(timeout=self.timeout, verify=not self.insecure_tls)) as client:
            ...
            if response is not None:
                ...
                if response.status_code == 204:
                    ...
                    if len(body_preview) > 300:
                        body_preview = body_preview[:300] + "..."
                    logger.error(
                        "Client login failed for '%s': HTTP %s, body=%s",
                        self.account.name,
                        response.status_code,
                        body_preview or "<empty>",
                    )
    ...
    async def chat_completion(
        ...
        try:
            ...
            for i, msg in enumerate(messages):
                ...
                if is_last_user:
                    # 如果最后一条消息是工具结果，说明这是“工具结果续问”。
                    # 这里不能丢掉最后一条 user，否则后续请求只剩 tool result
                    # 和通用续写提示，模型会失去当前任务目标。
                    if messages and messages[-1].get("role") == "tool":
                        if content:
                            history_messages.append({"role": role, "content": content})
                            logger.debug("Preserved latest user message in history for tool-result follow-up")
                    else:
                    ...
                    if role == "user":
                        ...
                        if msg.get("tool_calls"):
                            msg_dict["tool_calls"] = normalize_tool_calls(msg.get("tool_calls"))
            ...
            omit_tools_for_tool_results = False
            try:
                import sys
                if 'server' in sys.modules:
                    ...
                    omit_tools_for_tool_results = settings.get('omit_tools_for_tool_results', False) if settings else False
            ...
            # 第二轮/后续轮默认继续保留 tools。
            # 之前无条件省略 follow-up tools 会让上游失去当前工具定义，进而回落到旧工具名。
            # 仅在显式打开兼容开关时才恢复旧行为。
            request_tools = tools
            if tool_results and tools and omit_tools_for_tool_results:
                request_tools = None
                logger.debug("Omitting tool definitions for tool-result follow-up request")
            elif tool_results and tools:
                logger.debug("Keeping tool definitions for tool-result follow-up request")
            trace_request_base = {
                "model": model,
                "stream": stream,
                ...
                "tool_results": [summarize_tool_result(tool_result) for tool_result in tool_results],
                "request_tool_count": len(request_tools or []),
                "request_tools": [summarize_tool_definition(tool) for tool in (request_tools or [])],
                "omitted_tools_for_tool_results": bool(tool_results and tools and request_tools is None),
            }
            ...
            if split_toolcall_result and tool_results and len(tool_results) > 1:
                ...
                for idx, single_tool_result in enumerate(tool_results):
                    ...
                    self._log_trace(
                        trace_id,
                        "warp_request_out",
                        split_tool_result_index=idx + 1,
                        split_tool_result_total=len(tool_results),
                        protobuf_size=len(protobuf_bytes),
                        request={
                            **trace_request_base,
                            "history_count": len(accumulated_history[:len(history_messages) + idx + 1]),
                            "history_messages": [
                                summarize_message(message)
                                for message in accumulated_history[:len(history_messages) + idx + 1]
                            ],
                            "tool_result_count": 1,
                            "tool_results": [summarize_tool_result(single_tool_result)],
                        },
                    )

                    new_conversation_id = None
                    async for event in self._send_request_and_parse(protobuf_bytes, model, trace_id=trace_id):
    ...
    async def _send_request_and_parse(
        self,
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                for transport_attempt in range(1, 3):
                    try:
                        ...
                        break
                    except (httpx.TransportError, httpx.TimeoutException) as transport_error:
                        ...
                        self._log_trace(
                            ...
                            error=self._format_httpx_exception(transport_error),
                        )
                        if transport_attempt >= 2:
                            raise
                        logger.warning(
                            "Transport error talking to Warp for '%s' (attempt %s/2): %s [%s]; retrying",
                            self.account.name,
                            transport_attempt,
                            transport_error,
                            type(transport_error).__name__,
                        )
                        await asyncio.sleep(0.5 * transport_attempt)
                ...
                async for event in self._parse_sse_stream(response):
                    ...
                    self._log_trace(
                        ...
                        warp_event=summarize_warp_event(event),
                    )
            ...
            logger.error(
                "Error in chat_completion for '%s': %s [%s]",
                self.account.name,
                e,
                type(e).__name__,
            )
