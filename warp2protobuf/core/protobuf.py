...
def _build_descset(proto_files: List[str], includes: List[str]) -> bytes:
    protoc_main = None
    external_protoc = None
    try:
        from grpc_tools import protoc as grpc_protoc
        protoc_main = grpc_protoc.main
    except ModuleNotFoundError:
        external_protoc = shutil.which("protoc")
    ...
    if protoc_main is not None:
        rc = protoc_main(args)
        if rc != 0 or not out.exists():
            raise RuntimeError("grpc_tools.protoc failed to produce descriptor set")
        return out.read_bytes()

    if external_protoc:
        cmd = [external_protoc, *args[1:]]
        proc = subprocess.run(cmd, capture_output=True, text=True)
        if proc.returncode != 0 or not out.exists():
            stderr = proc.stderr.strip() or proc.stdout.strip()
            raise RuntimeError(f"protoc failed to produce descriptor set: {stderr}")
        return out.read_bytes()

    raise RuntimeError(
        "protobuf runtime requires 'grpcio-tools' or a 'protoc' executable in PATH. "
...
def build_request_bytes(
    ...
    统一使用 protobuf builder 构建请求。模板字节只保留作为调试/回归时的备用实现。
    """
    return build_request_bytes_with_history(
        user_text,
        model,
        disable_warp_tools,
        history_messages,
...
def build_request_bytes_with_history(
    ...
    # 该 builder 同时处理新会话和带历史的请求。
    # 当 history/task_id/tool_results 都为空时，构造一个真正的新会话请求；
    # 避免继续依赖脆弱的模板字节拼接逻辑。
    ...
    settings.model_config.base = model_config.get("base", "auto-genius")
    # planning 字段留空（不设置）
