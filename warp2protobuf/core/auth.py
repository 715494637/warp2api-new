...
async def refresh_jwt_token() -> dict:
    ...
    try:
        async with httpx.AsyncClient(**get_httpx_client_kwargs(timeout=30.0)) as client:
...
def persist_refresh_response(token_data: dict) -> bool:
    access_token = token_data.get("access_token") or token_data.get("idToken")
    if not access_token:
        return False

    success = update_env_file(access_token)
    new_refresh_token = token_data.get("refresh_token") or token_data.get("refreshToken")
    if new_refresh_token:
        success = update_env_refresh_token(new_refresh_token) and success
    return success
...
async def check_and_refresh_token() -> bool:
    ...
    if not current_jwt:
        ...
        if token_data:
            return persist_refresh_response(token_data)
        return False
    ...
    if is_token_expired(current_jwt, buffer_minutes=15):
        ...
        token_data = await refresh_jwt_token()
        if token_data:
            new_jwt = token_data.get("access_token") or token_data.get("idToken")
            if not new_jwt:
                logger.error("Refresh response missing access token")
                return False
...
async def _create_anonymous_user() -> dict:
    ...
    async with httpx.AsyncClient(**get_httpx_client_kwargs(timeout=httpx.Timeout(30.0))) as client:
...
async def acquire_anonymous_access_token() -> str:
    ...
    async with httpx.AsyncClient(**get_httpx_client_kwargs(timeout=httpx.Timeout(30.0))) as client:
        ...
        persist_refresh_response(token_data)
        return access
